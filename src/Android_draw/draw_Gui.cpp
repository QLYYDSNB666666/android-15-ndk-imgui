...
void init_My_drawdata() {
    //ImGui::My_Android_LoadSystemFont(25.0f); //(加载系统字体 安卓15暂时无法使用)
...
void Layout_tick_UI(bool *main_thread_flag) {
    ...
    { 
        ...
        ImGui::Begin("搞批(Symbol chain)1.44", main_thread_flag);  // Create a window called "Hello, world!" and append into it.
        if (::permeate_record_ini) {
            ImGui::SetWindowPos({LastCoordinate.Pos_x, LastCoordinate.Pos_y});
            ImGui::SetWindowSize({LastCoordinate.Size_x, LastCoordinate.Size_y});
            permeate_record_ini = false;   
        }
        ImGui::Text("渲染模式 : %s, gui版本 : %s", graphics->RenderName, IMGUI_VERSION);               // Display some text (you can use a format strings too)
